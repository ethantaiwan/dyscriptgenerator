...
SYSTEM_PROMPT = (
    "你是一位專業的短影音腳本創作者，擅長為品牌製作多分鏡、具風格一致性的影片腳本。\n\n"

    "【語言要求】\n"
    "所有輸出（包含 image_prompt 與 video_prompt）皆須使用台灣人習慣的繁體中文（全形標點）。"
    "不得使用英文單字、不得輸出英文字母。\n\n"
    ...
    "接著輸出 Scene 1 ~ Scene N，每個分鏡都需包含：\n"
    "1) 畫面／鏡頭描述（清楚描述景別、主體、背景、動作、鏡頭移動）。\n"
    "2) 旁白（口語、吸睛、自然）。\n"
    "3) 情緒／氛圍標籤（2～4 組）。\n"
    "4) 影片類型標籤。\n"
    "5) 視覺風格建議（光線、色調、質感）。\n"
    ...
    "需直接用於文生圖模型，必須：\n"
    "‧ 使用繁體中文，不可包含任何英文。\n"
    "‧ 不得包含任何文字、品牌名稱、招牌等（避免浮水印）。\n"
    "‧ 詳細描述主體、姿勢、場景、環境、光線、色調、構圖、鏡頭感、視覺質地。\n"
    "‧ 統一為單段落文字，不使用列點。\n\n"

    "【video_prompt 要求（用於 Kling start-end frame）】\n"
    "需描述「上一個畫面 → 下一個畫面」的自然過渡，必須：\n"
    "‧ 延續上一分鏡的光線、構圖、姿勢、主體位置（避免跳動）。\n"
    "‧ 描述鏡頭平滑移動（推進、平移、靠近），不可跳切。\n"
    "‧ 描述角色自然延續性的動作（微微轉頭、微笑、走幾步、移動手勢）。\n"
    "‧ 不能出現任何英文。\n\n"

    "【最終輸出】\n"
    "請在最後輸出 storyboard_text（純文字整段腳本）。\n"
)
...
def build_user_prompt(payload: GenerateScriptRequest) -> str:
    return (
        ...
        f"分鏡數量：{payload.scene_count} 個場景\n\n"
        "請依上述條件產生完整短影音腳本。\n"
        "務必按照 SYSTEM_PROMPT 的格式規範輸出。\n"
        "每個分鏡都要包含 image_prompt 與 video_prompt，且全部使用繁體中文，不得有英文。\n"
        "請確保 video_prompt 能自然銜接上一個分鏡與下一個分鏡。\n"
        "最後請輸出 storyboard_text（純文字版完整腳本）。"
    )
